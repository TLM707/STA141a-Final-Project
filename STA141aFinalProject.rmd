...
```{r Session 1 cont, include=FALSE}
...
```{r Graphs SESSION 1 comparing success and failure rates per session, echo=FALSE, message=FALSE, warning=FALSE}
# Extract spiking activity for session 1
spike_list <- lapply(seq_along(session[[1]]$spks), function(t) {
  mat_1 <- session[[1]]$spks[[t]]
  feedback_1 <- session[[1]]$feedback_type[t]
  
  as.data.frame(mat_1) %>%
    mutate(neuron = 1:nrow(mat_1)) %>%
    pivot_longer(cols = -neuron, names_to = "time_bin", values_to = "spike_count") %>%
    mutate(feedback_type = feedback_1, session_ID = 1)
})

# Combine into a single dataframe
spike_data_1 <- bind_rows(spike_list) %>%
  mutate(time_bin = as.numeric(gsub("V", "", time_bin)))

# Compute average spike count per time bin for each feedback type
psth_data <- spike_data_1 %>%
  group_by(time_bin, feedback_type) %>%
  summarise(avg_spike_count = mean(spike_count, na.rm = TRUE), .groups = "drop")

# Generate plot
p_1 <- ggplot(psth_data, aes(x = time_bin, y = avg_spike_count, color = factor(feedback_type))) +
  geom_line(size = 1) +
  scale_color_manual(values = c("#DE2D26", "#1F78B4"), labels = c("Failure", "Success")) +
  labs(title = "Peri-Stimulus Time Histogram (PSTH) - Session 1",
...
```{r SESSION 2 comparing success and failure, echo=FALSE, message=FALSE, warning=FALSE}
spike_list_2 <- lapply(seq_along(session[[2]]$spks), function(t) {
  mat_2 <- session[[2]]$spks[[t]]
  feedback_2 <- session[[2]]$feedback_type[t]
  
  as.data.frame(mat_2) %>%
    mutate(neuron = 1:nrow(mat_2)) %>%
    pivot_longer(cols = -neuron, names_to = "time_bin", values_to = "spike_count") %>%
    mutate(feedback_type = feedback_2, session_ID = 2)
})

# Combine into a single dataframe
spike_data_2 <- bind_rows(spike_list_2) %>%
  mutate(time_bin = as.numeric(gsub("V", "", time_bin)))

# Compute average spike count per time bin for each feedback type
psth_data_2 <- spike_data_2 %>%
  group_by(time_bin, feedback_type) %>%
  summarise(avg_spike_count = mean(spike_count, na.rm = TRUE), .groups = "drop")

# Generate plot
p_2 <- ggplot(psth_data_2, aes(x = time_bin, y = avg_spike_count, color = factor(feedback_type))) +
  geom_line(size = 1) +
  scale_color_manual(values = c("#DE2D26", "#1F78B4"), labels = c("Failure", "Success")) +
  labs(title = "Peri-Stimulus Time Histogram (PSTH) - Session 2",
       x = "Time Bins", y = "Average Spike Count", color = "Feedback Type") +
  theme_minimal()
...
```{r SESSION 3 comparing success and failure, echo=FALSE, message=FALSE, warning=FALSE}
spike_list_3 <- lapply(seq_along(session[[3]]$spks), function(t) {
  mat_3 <- session[[3]]$spks[[t]]
  feedback_3 <- session[[3]]$feedback_type[t]
  
  as.data.frame(mat_3) %>%
    mutate(neuron = 1:nrow(mat_3)) %>%
    pivot_longer(cols = -neuron, names_to = "time_bin", values_to = "spike_count") %>%
    mutate(feedback_type = feedback_3, session_ID = 3)
})

# Combine into a single dataframe
spike_data_3 <- bind_rows(spike_list_3) %>%
  mutate(time_bin = as.numeric(gsub("V", "", time_bin)))

# Compute average spike count per time bin for each feedback type
psth_data_3 <- spike_data_3 %>%
  group_by(time_bin, feedback_type) %>%
  summarise(avg_spike_count = mean(spike_count, na.rm = TRUE), .groups = "drop")

# Generate plot
p_3 <- ggplot(psth_data_3, aes(x = time_bin, y = avg_spike_count, color = factor(feedback_type))) +
  geom_line(size = 1) +
  scale_color_manual(values = c("#DE2D26", "#1F78B4"), labels = c("Failure", "Success")) +
  labs(title = "Peri-Stimulus Time Histogram (PSTH) - Session 3",
       x = "Time Bins", y = "Average Spike Count", color = "Feedback Type") +
  theme_minimal()
